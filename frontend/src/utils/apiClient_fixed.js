...
export const testConnection = async (baseURL, apiKey, apiSecret) => {
  try {
    ...
    const approaches = [
      ...
      () => {
        const client = createApiClient(cleanBaseURL);
        return client.get('/api/method/frappe.auth.get_logged_user', {
          headers: {
      ...
      // Approach 2: Using proxy (if available)
      () => {
        const client = createApiClient('');
        return client.get(`${cleanBaseURL}/api/method/frappe.auth.get_logged_user`, {
          headers: {
            'Authorization': `token ${apiKey}:${apiSecret}`,
            'Content-Type': 'application/json',
            'Accept': 'application/json'
      ...
      // Approach 3: Simple fetch with CORS mode
      () => {
        return fetch(`${cleanBaseURL}/api/method/frappe.auth.get_logged_user`, {
          method: 'GET',
          mode: 'cors',
          headers: {
            'Authorization': `token ${apiKey}:${apiSecret}`,
